// src/app/api/scheduled-payments/due/route.ts (ULTRA-SAFE VERSION)
...
export async function GET(request: NextRequest) {
  try {
    ...
    const duePayments = await db
        ...
        $and: [
          {
            $or: [
              ...
              {
                processingStarted: {
                  $lt: twoMinutesAgo, // Processing started more than 2 minutes ago (stale)
                },
              },
            ],
          },
          ...
          // Must NOT have been executed recently
          {
            $or: [
              ...
              {
                lastExecutionAt: {
                  $lt: oneMinuteAgo, // Last execution was more than 1 minute ago
                },
              },
            ],
          },

          // Must NOT have been created very recently (prevent immediate execution)
          {
            $or: [
              { createdAt: { $exists: false } }, // No creation date
              {
                createdAt: {
                  $lt: new Date(now.getTime() - 30000), // Created more than 30 seconds ago
                },
              },
            ],
          },

          // Must NOT have been updated very recently (prevent re-execution during updates)
          {
            $or: [
              { updatedAt: { $exists: false } }, // No update date
              {
                updatedAt: {
                  $lt: new Date(now.getTime() - 30000), // Updated more than 30 seconds ago
                },
              },
            ],
          },
      ...
      .sort({ nextExecutionAt: 1 }) // Process oldest due payments first
      .limit(10) // Limit to 10 payments at a time
      .toArray();
    ...
    const transformedPayments = safeDuePayments.map((payment) => {
      ...
      // Log details for each payment we're returning
      console.log(
