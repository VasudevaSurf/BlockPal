// Quick Fix API Endpoint to Handle Stuck Processing Payments
// src/app/api/scheduled-payments/fix-stuck/route.ts (NEW FILE)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const stuckPayments = await db
      ...
      .find({
        status: "processing",
        processingStarted: { $lt: fiveMinutesAgo }, // Processing started more than 5 minutes ago
        username: decoded.username, // Only user's own payments
      })
      .toArray();
        ...
        if (payment.frequency === "once") {
          ...
          const updateResult = await db.collection("schedules").updateOne(
            { _id: payment._id },
            {
              $set: {
                status: "completed",
                executedCount: 1, // Assume it was executed once
                lastExecutionAt: payment.processingStarted, // Use processing start time as execution time
                completedAt: now,
                updatedAt: now,
